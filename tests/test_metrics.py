...
def test_metric_label_names():
    ...
    assert SOLR_QUERY_DURATION._labelnames == ()
...
async def test_solr_query_records_success_metrics(sample_solr_response):
    ...
    before_duration = _get_histogram_count("okp_solr_query_duration_seconds")
    ...
    assert _get_histogram_count("okp_solr_query_duration_seconds") == before_duration + 1
...
async def test_solr_query_always_records_duration():
    ...
    before = _get_histogram_count("okp_solr_query_duration_seconds")
    ...
    assert _get_histogram_count("okp_solr_query_duration_seconds") == before + 1
